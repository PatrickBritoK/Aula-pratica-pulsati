...
app.post("/aeroporto", (req, res) =>{
    ...
    const Aeroporto = new aeroporto(proximoCodigo,nome,codigoAeroporto,endereco);
...
app.get("/aeroporto/:codigo", (req, res) => {
  ...
  app.put("/aeroporto/:codigo", (req, res) => {
    const codigo = req.params.codigo;
    const aeroporto = aeroportoList.find(aeroportoAtual => aeroportoAtual.codigo == codigo);
    const { nome, codigoAeroporto, endereco } = req.body;
    aeroporto.nome = nome;
